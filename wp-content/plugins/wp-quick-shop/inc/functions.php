<?php if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly
		...
		if(!empty($_POST)){
			if ( 
				...
				if(isset($_POST['var']) && !empty($_POST['var'])){
                    ...
                    $wpqs_enabled = wpqs_enabled();
                    $wpqs_variable_array = isset($_POST['var']) ? $_POST['var'] : array();
	...
	function wpqs_plugin_links($links) { 
		...
		if($wpqs_pro){
			...
			$wpqs_premium_link = '<a href="'.$wpqs_premium_link.'" title="'.__('Go Premium', 'wp-quick').'" target=_blank>'.__('Go Premium', 'wp-quick').'</a>'; 
	...
	function wpqs_admin_style() {
		...
		wp_register_style('wpqs-admin', plugins_url('css/admin-style.css', dirname(__FILE__)));
	...
	function wpqs_enabled(){
		...
		if(in_array( 'woocommerce-discounts-plus/index.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )){
